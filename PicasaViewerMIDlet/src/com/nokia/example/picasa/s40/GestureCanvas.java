...
public abstract class GestureCanvas extends Canvas {
    ...
    /**
     * The user finished a animateDrag motion by lifting their finger when not moving
     * 
     * @param startX
     * @param startY
     * @param dragDistanceX
     ...
     * 
     * The default implementation does kinetic scrolling on both X and Y. You can
     * reduce the computational load and thus slightly increase the frame rate by
     * overriding this if you are only interested in animation on one axis.
     * 
     * @param startX
     * @param startY
    ...
    /**
     * Update your GameCanvas by painting at the new scroll position
     * 
     * @param x
     * @param y
     * @param delta
     ...
     * @param lastFrame 
     */
    abstract public void animate(int x, int y, short delta, short deltaX, short deltaY, boolean lastFrame);
    ...
    public synchronized void startSpin(final long delay) {
        ...
        if (spinTimerTask == null) {
            ...
            spinTimer.schedule(spinTimerTask, delay);
        }
    }
